...
void main() {
  ...
  vec4 color = v_color0;
    ...
    diffuse.rgb -= mix(vec3(0.0, 0.0, 0.0), texture2D(s_MatTexture, v_texcoord0 + offset*0.13).rgb, 1.9);
  #endif
  ...
  // experimental (break lighting)
  #ifdef NL_SHARP_SHADOW
    vec2 uvl = v_lightmapUV;
    float shadowmap = smoothstep(0.915, 0.890, uvl.y);
    diffuse.rgb *= mix(vec3(1.0, 1.0, 1.0), vec3(0.3, 0.4, 0.425), shadowmap);
